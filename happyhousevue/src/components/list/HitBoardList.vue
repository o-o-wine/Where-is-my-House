<template>
  ...
  <v-div class="list-component">
    <v-data-table :headers="headers" :items="hotboard" disable-sort :items-per-page="itemsPerPage" hide-default-footer
      ...
      <template v-slot:top>
        <v-toolbar flat class="transparent" style="color:white; font-weight: bold;">
          <v-toolbar-title>인기글</v-toolbar-title>
...
export default {
  ...
  async created() {
    await this.$store.dispatch("getHotBoard");
  },
  watch: {
    dialog(val) {
      val || this.close();
